...
const EditPartSelect = ({ partId, setPartId, data }) => {
  const [ userBuilds, setUserBuilds ] = useState([]);
  const [ userParts, setUserParts ] = useState([]);
  const [ buildId, setBuildId ] = useState(data.part.builds[0]?.id || false);
  const [ catId, setCatId ] = useState(data.part.part_type.categoryId);
  ...
  const handlePartOptions = () => {
    try {// Only proceed if both build id and category id have been selected by user
      if (buildId && catId && userParts.length !== 0 && userBuilds.length !== 0) {
  ...
  useEffect(() => {
    handlePartOptions();
    // setPartId('');
  }, [buildId, catId, userBuilds, userParts]);
  ...
  return (
      <div className='select-container'> {/* Build Select */}
        <label className='select-label' htmlFor="build">Build:</label>
        <select 
          className='select'
          value={buildId}
          name="build" 
        ...
        <div className='nested-select-container'>
          <label className='select-label' htmlFor="category">Category:</label>
          <select
            className='select'
            value={catId}
            name="category" 
